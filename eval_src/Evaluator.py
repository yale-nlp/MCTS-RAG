...
class CWEBQAEvaluator(Evaluator):
    def __init__(self) -> None:
        super().__init__()
    ...
    def check_answers_equiv(self, answer_a: str, answer_b: str):
        ...
        # io_input = f"Answer a is acceptable if it has the core content of expected answer.\n\nIs answer A an acceptable answer? A is: {format_answer_a}. The expected answer is: {format_answer_b}. Only generate True or False"
        # print(f"eval two answer: {format_answer_a} and {format_answer_b}")
        # io_output = generate_with_OpenAI_model(prompt=io_input, model_ckpt="gpt-4o")
        
        # if io_output[0] == "True":
        #     print(f"eval result: ", True)
        #     return True
        # else:
        #     return False
        # from sentence_transformers import SentenceTransformer, util
# 
        # model = SentenceTransformer('all-MiniLM-L6-v2')  # 使用轻量模型
# 
        # def are_answers_semantically_similar(answer_a, answer_b, threshold=0.7):
        #     embeddings = model.encode([answer_a, answer_b])
        #     similarity = util.cos_sim(embeddings[0], embeddings[1])
        #     return similarity >= threshold
        # 
        # return are_answers_semantically_similar(answer_a, answer_b)
        
        return format_answer_a == format_answer_b or fuzz.token_sort_ratio(format_answer_a, format_answer_b) >= 45 or format_answer_a in format_answer_b or format_answer_b in format_answer_a
...
# 
        ...
        return format_answer_a == format_answer_b or fuzz.token_sort_ratio(format_answer_a, format_answer_b) >= 85 or format_answer_a in format_answer_b or format_answer_b in format_answer_a
...
class ScienceQAEvaluator(Evaluator):
    def __init__(self) -> None:
        super().__init__()
...
class SciKEvalEvaluator(Evaluator):
    def __init__(self) -> None:
        super().__init__()
...
class FMTEvaluator(Evaluator):
    ...
    def check_answers_equiv(self, answer_a: str, answer_b: str):
        ...
        format_answer_a = self._format_answer(answer_a)
        format_answer_b = self._format_answer(answer_b)
        if format_answer_a in ["a", "supports", "support"]:
            format_answer_a = "yes"
        elif format_answer_a in ["b", "refute", "refutes"]:
            format_answer_a = "no"
        if format_answer_b in ["a", "supports", "support"]:
            format_answer_b = "yes"
        elif format_answer_b in ["b", "refute", "refutes"]:
            format_answer_b = "no"
        return format_answer_a == format_answer_b or fuzz.token_sort_ratio(format_answer_a, format_answer_b) >= 90
